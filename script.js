document.addEventListener('DOMContentLoaded', function () {
      ...
      if (field.hasAttribute('required') && value === '') {
        ...
        if (type === 'url') {
          try {
            ...
            fieldValid = false;
            errorMessage = getErrorMessage(field, 'url');
          }
        }
        ...
        if (field.hasAttribute('minlength')) {
          ...
          if (value.length < min) {
            fieldValid = false;
            errorMessage = getErrorMessage(field, 'minlength', min);
          }
        }
        ...
        if (field.hasAttribute('maxlength')) {
          ...
          if (value.length > max) {
            fieldValid = false;
            errorMessage = getErrorMessage(field, 'maxlength', max);
          }
        }
        ...
        if (field.hasAttribute('pattern')) {
          ...
          if (!pattern.test(value)) {
            fieldValid = false;
            errorMessage = getErrorMessage(field, 'pattern');
          }
        }
      }
  ...
  function getErrorMessage(field, type, param) {
    ...
    const messages = {
      ...
      maxlength: label + ' must not exceed ' + param + ' characters.',
  ...
  function getFieldLabel(field) {
    ...
    const name = field.name || field.placeholder || 'This field';
  ...
  function getErrorElement(field) {
    ...
    return parent.querySelector('.form-error-message');
  }

  function clearFormErrors(form) {
    const errorFields = form.querySelectorAll('.field-error, .field-success');
  ...
  function handleFormSuccess(form) {
    ...
    setTimeout(function () {
      form.reset();
      const successFields = form.querySelectorAll('.field-success');
      successFields.forEach(function (f) {
        f.classList.remove('field-success');
      });
    }, 500);
  }
  ...
  const animationTargetSelectors = [
    '.fade-in',
    '.animate-on-scroll',
    '[data-animate]',
    '.feature-card',
  ...
  function initScrollAnimations() {
    if (!('IntersectionObserver' in window)) {
      animationTargetSelectors.forEach(function (selector) {
    ...
    uniqueElements.forEach(function (el) {
      ...
      if (group) {
        if (!staggerGroups[group]) {
          staggerGroups[group] = 0;
        }
        if (!el.hasAttribute('data-delay')) {
          el.setAttribute('data-delay', (staggerGroups[group] * 0.1).toString());
        }
        staggerGroups[group]++;
      }
      ...
      if (parent) {
        const siblings = parent.querySelectorAll(animationTargetSelectors.join(','));
        if (siblings.length > 1 && !el.hasAttribute('data-delay') && !group) {
          const siblingArray = Array.from(siblings);
          const index = siblingArray.indexOf(el);
          if (index > 0) {
            el.setAttribute('data-delay', (index * 0.1).toString());
          }
        }
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "BreezIL משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '43768fab-b9ea-4b48-ae46-1c427de59d59',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '43768fab-b9ea-4b48-ae46-1c427de59d59';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '43768fab-b9ea-4b48-ae46-1c427de59d59';
